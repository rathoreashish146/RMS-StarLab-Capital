...
# ---------- Helpers ----------
def current_user():
    """Return logged-in User or None. Safe for SQLAlchemy 2.x."""
    try:
        uid = session.get("user_id")
        if not uid:
            return None
        with SessionLocal() as s:
            # old: s.query(User).options(...).get(uid)  # <-- breaks on SA 2.x
            u = s.get(User, uid)  # SA 2.x compatible
            if not u:
                return None
            # warm up relationship while session is open
            _ = u.office.name if u.office else None
            return u
    except Exception:
        # never let this crash the first render
        return None
...
def load_profile_remarks(_):
    ...
    with SessionLocal() as s:
        emp = _employee_for_user(user, s)
        if not emp:
            return html.Div("No remarks.")
        remarks = s.query(Remark).filter(
            Remark.target_type == "EMPLOYEE",
            Remark.target_id == emp.id
        ).order_by(Remark.created_at.desc()).all()
        if not remarks:
            return html.Div("No remarks yet.")
        items = []
        for r in remarks:
            ts = r.created_at.strftime("%Y-%m-%d %H:%M")
            items.append(html.Li(f"[{ts}] {r.content}"))
