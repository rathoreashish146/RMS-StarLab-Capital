...
def navbar(current_path="/"):
    ...
    if user.role == Role.EMP:
        nav_items = [
            dcc.Link("💼 My Assets", href="/assets", title="View and manage your assets", className="active" if current_path == "/assets" else ""),
            dcc.Link("📝 Requests", href="/requests", title="Submit and track resource requests", className="active" if current_path == "/requests" else ""),
            dcc.Link("👤 My Profile", href="/profile", title="Update your profile information", className="active" if current_path == "/profile" else ""),
        ]
    else:
        nav_items = [
            dcc.Link("📊 Dashboard", href="/", title="View system overview", className="active" if current_path in ("/", None) else ""),
            dcc.Link("💼 Assets", href="/assets", title="Manage all assets", className="active" if current_path == "/assets" else ""),
            dcc.Link("📝 Requests", href="/requests", title="Review and approve requests", className="active" if current_path == "/requests" else ""),
            dcc.Link("📈 Reports", href="/reports", title="View analytics and reports", className="active" if current_path == "/reports" else ""),
        ]
        if user.role == Role.GM:
            nav_items.append(dcc.Link("🏢 Offices", href="/offices", title="View and manage offices", className="active" if current_path == "/offices" else ""))
            nav_items.append(dcc.Link("⚙️ Admin", href="/admin", title="System administration", className="active" if current_path == "/admin" else ""))
        else:
            nav_items.append(dcc.Link("👥 Employees", href="/employees", title="Manage your team", className="active" if current_path == "/employees" else ""))
...
def assets_layout():
    ...
    # Employee view - Read-only assets list
    if user.role == Role.EMP:
        return html.Div([
            navbar("/assets"),
            html.Div(className="card", children=[
                html.H3("💼 My Assets", style={"marginBottom": "8px"}),
                html.Div("View all resources assigned to you. To request new assets, use the 'Requests' page.", className="muted", style={"marginBottom": "24px", "fontSize": "14px"}),
                html.Div(style={"display": "flex", "alignItems": "center", "justifyContent": "space-between", "marginBottom": "16px"}, children=[
                    html.H4("📋 My Assets List", style={"margin": "0"}),
    ...
    # Manager/GM view - Full asset management
    ...
    return html.Div([
        ...
        html.Div(className="card", children=[
            ...
            _uploader_component("upload-bill"),
                html.Div(id="upload-bill-status", style={"marginTop": "8px"}),
            ]),
...
def reports_layout():
    ...
    if not user:
        return login_layout()
    if user.role == Role.EMP:
        return html.Div([navbar("/reports"), html.Div(className="card", children=[
...
def employees_layout():
    ...
    if user.role != Role.OM:
        return html.Div([navbar("/employees"), html.Div(className="card", children=[
            html.Div(style={"textAlign": "center", "padding": "40px"}, children=[
...
def admin_layout():
    ...
    if not user or user.role != Role.GM:
        return html.Div([navbar("/admin"), html.Div(className="card", children=[
            html.Div(style={"textAlign": "center", "padding": "40px"}, children=[
    ...
    return html.Div([
        ...
        html.Div(className="card", children=[
            ...
            html.H4("👤 Create Employee", style={"marginBottom": "8px"}),
            html.Div("Create employees and assign them to offices with specific roles.", className="muted", style={"marginBottom": "12px", "fontSize": "13px"}),
            html.Div(className="two-col", children=[
                html.Div(className="form-group", children=[
                    html.Label("Employee Name *", className="form-label"),
                    dcc.Input(id="emp-name", placeholder="Full name", className="input", style={"maxWidth": "100%"}),
                ]),
                html.Div(className="form-group", children=[
                    html.Label("Username *", className="form-label"),
                    dcc.Input(id="emp-username", placeholder="Login username", className="input", style={"maxWidth": "100%"}),
                ]),
            ]),
            html.Div(className="two-col", children=[
                html.Div(className="form-group", children=[
                    html.Label("Password *", className="form-label"),
                    dcc.Input(id="emp-password", type="password", placeholder="Initial password", className="input", style={"maxWidth": "100%"}),
                ]),
                html.Div(className="form-group", children=[
                    html.Label("Phone (Optional)", className="form-label"),
                    dcc.Input(id="emp-phone", placeholder="Phone number", className="input", style={"maxWidth": "100%"}),
                ]),
            ]),
            html.Div(className="two-col", children=[
                html.Div(className="form-group", children=[
                    ...
                    dcc.Dropdown(
                        ...
                        options=[
                            ...
                            {"label": "Office Manager (OM)", "value": "OM"},
                            {"label": "Employee (EMP)", "value": "EMP"}
                        ],
                ...
                html.Div(className="form-group", children=[
                    html.Label("Office *", className="form-label"),
                    dcc.Dropdown(id="emp-office", placeholder="Choose office", className="dash-dropdown", style={"maxWidth": "100%"}),
                ]),
            ]),
            html.Button("➕ Create Employee", id="btn-create-emp", className="btn"),
            html.Div(id="msg-create-emp", style={"marginTop":"12px"}),
            
            html.Div(className="hr"),
            
            html.H4("📋 All Employees", style={"marginBottom": "8px"}),
            html.Div("View all employees across the organization.", className="muted", style={"marginBottom": "12px", "fontSize": "13px"}),
            html.Div(id="admin-employees-table"),
...
def offices_layout():
    user = current_user()
    if not user or user.role != Role.GM:
        return html.Div([navbar("/offices"), html.Div(className="card", children=[
            html.Div(style={"textAlign": "center", "padding": "40px"}, children=[
...
def route(path):
    ...
    if not user:
        return login_layout()
    if path in ("/", None):
        # Redirect employees to assets page (no dashboard for them)
        if user.role == Role.EMP:
    ...
    return html.Div([navbar(path), html.Div(className="card", children=html.H3("Not Found"))])
...
# ---------- Assets ----------
@app.callback(
    Output("upload-bill-status", "children"),
    Input("upload-bill", "contents"),
    State("upload-bill", "filename"),
    prevent_initial_call=True
)
def show_upload_status(contents, filename):
    """Display upload status when file is selected"""
    if contents and filename:
        return html.Div([
            html.Span("✅ ", style={"fontSize": "16px"}),
            html.Span("Document uploaded: ", style={"fontWeight": "600", "color": "var(--success)"}),
            html.Span(filename, style={"color": "var(--text-secondary)", "fontSize": "13px"})
        ], style={
            "padding": "10px 14px",
            "background": "var(--success-light)",
...
@app.callback(
    ...
    Output("upload-bill-status", "children", allow_duplicate=True),
...
def add_asset(n, name, price, qty, contents, filename, alloc_type, alloc_target):
    ...
    if not name:
        return (html.Div("❌ Asset name is required.", className="message message-error"), render_assets_table(), "", False, name, price, qty, contents, html.Div())
    if price_val <= 0:
        return (html.Div("❌ Price must be greater than 0.", className="message message-error"), render_assets_table(), "", False, name, price, qty, contents, html.Div())
    if qty_val < 1:
        return (html.Div("❌ Quantity must be at least 1.", className="message message-error"), render_assets_table(), "", False, name, price, qty, contents, html.Div())
    ...
    with SessionLocal() as s:
        ...
        s.commit()
    return (html.Div("✅ Asset added successfully!", className="message message-success"), render_assets_table(), "Asset added.", True, "", "", 1, None, html.Div())
    ...
    with SessionLocal() as s:
        ...
        if not assets:
            return html.Div(className="empty-state", children=[
                html.H3("No Assets Found"),
                html.Div("Add your first asset using the form above." if user.role != Role.EMP else "No assets have been assigned to you yet.", className="muted")
            ])
        
        rows = []
        for a in assets:
            allocated_to = "—"
            if a.allocation_type == AllocationType.EMPLOYEE and a.allocation_id:
                emp = s.get(Employee, a.allocation_id)
                allocated_to = emp.name if emp else f"Employee #{a.allocation_id}"
            elif a.allocation_type == AllocationType.OFFICE and a.allocation_id:
                office = s.get(Office, a.allocation_id)
                allocated_to = office.name if office else f"Office #{a.allocation_id}"
            elif a.allocation_type == AllocationType.UNALLOCATED:
            ...
            rows.append({
                ...
                "price": f"${a.price:,.2f}",
                "qty": a.quantity,
                "total": f"${(a.price * a.quantity):,.2f}",
                "bill": _bill_link_asset(a),
                "allocation": a.allocation_type.value.replace("_", " ").title(),
                "allocated_to": allocated_to,
                "status": "Returned" if a.returned else "Active"
            })

        cols = [
            {"name":"ID","id":"id"},
            {"name":"Asset Name","id":"name"},
            {"name":"Unit Price","id":"price"},
            {"name":"Qty","id":"qty"},
            {"name":"Total Value","id":"total"},
            {"name":"Bill/Receipt","id":"bill","presentation":"markdown"},
            {"name":"Allocation Type","id":"allocation"},
            {"name":"Allocated To","id":"allocated_to"},
        ...
        if user.role == Role.EMP:
            ...
            return html.Div([
                ...
                html.Div(className="form-group", children=[
                    ...
                    html.Div(id="req-bill-status", style={"marginTop": "8px"}),
                ]),
...
@app.callback(
    Output("req-bill-status", "children"),
    Input("req-bill", "contents"),
    State("req-bill", "filename"),
    prevent_initial_call=True
)
def show_request_upload_status(contents, filename):
    """Display upload status when request bill is selected"""
    if contents and filename:
        return html.Div([
            html.Span("✅ ", style={"fontSize": "16px"}),
            html.Span("Document uploaded: ", style={"fontWeight": "600", "color": "var(--success)"}),
...
@app.callback(
    ...
    Output("req-bill-status", "children", allow_duplicate=True),
...
def create_request(n, emp_id, asset_name, qty, price, contents, filename):
    ...
    if not asset_name:
        return html.Div("❌ Please enter an asset name.", className="message message-error"), render_requests_table(), "", False, asset_name, qty, price, contents, html.Div()
    if qty < 1:
        return html.Div("❌ Quantity must be at least 1.", className="message message-error"), render_requests_table(), "", False, asset_name, qty, price, contents, html.Div()
    ...
    with SessionLocal() as s:
        ...
        if not emp_id:
            return html.Div("❌ Select an employee.", className="message message-error"), render_requests_table(), "", False, asset_name, qty, price, contents, html.Div()
        emp = s.get(Employee, emp_id)
        if not emp:
            return html.Div("❌ Invalid employee.", className="message message-error"), render_requests_table(), "", False, asset_name, qty, price, contents, html.Div()
        if user.role == Role.OM and emp.office_id != user.office_id:
            return html.Div("❌ You can only submit requests for your office.", className="message message-error"), render_requests_table(), "", False, asset_name, qty, price, contents, html.Div()
        ...
        s.commit()
    return html.Div("✅ Request submitted successfully!", className="message message-success"), render_requests_table(), "Request submitted.", True, "", 1, "", None, html.Div()
...
def render_requests_table(_=None):
    ...
    with SessionLocal() as s:
        ...
        if not rows:
            return html.Div(className="empty-state", children=[
                html.H3("No Requests Yet"),
                html.Div("Submit your first request using the form above." if user.role == Role.EMP else "No requests have been submitted yet.", className="muted")
            ])
        
        data = []
        for r in rows:
            emp = s.get(Employee, r.employee_id) if r.employee_id else None
            office = s.get(Office, r.office_id) if r.office_id else None
            ...
            status_display = status_map.get(r.status.value, r.status.value)
            
            data.append({
                "id": r.id,
                "employee": emp.name if emp else f"Employee #{r.employee_id}",
                "office": office.name if office else f"Office #{r.office_id}",
                "asset": r.asset_name,
                "qty": r.quantity,
                ...
                "remark": r.remark or "—",
                "bill": _bill_link_path(r.bill_path) if r.bill_path else "—",
    ...
    table = dash_table.DataTable(
        ...
        style_data_conditional=[
            {
                'if': {'filter_query': '{status_raw} = PENDING'},
                'backgroundColor': 'var(--warning-light)',
            },
            {
                'if': {'filter_query': '{status_raw} = APPROVED'},
                'backgroundColor': 'var(--success-light)',
            },
            {
                'if': {'filter_query': '{status_raw} = REJECTED'},
                'backgroundColor': 'var(--danger-light)',
            },
            {
                'if': {'filter_query': '{status_raw} = RETURNED'},
                'backgroundColor': 'var(--muted)',
                'color': 'white',
                'opacity': '0.8'
            }
...
def _render_emp_table_for_om(user):
    with SessionLocal() as s:
        ...
        if not emps:
            return html.Div(className="empty-state", children=[
                html.H3("No Employees Yet"),
                html.Div("Add your first employee using the form above.", className="muted")
            ])
        data = [{
            "id": e.id, 
            "name": e.name, 
...
def add_employee(n, name, phone, uname, pwd):
    ...
    with SessionLocal() as s:
        ...
        emp = Employee(name=name, office_id=user.office_id, username=uname, role=Role.EMP)
...
def render_offices_table(_):
    ...
    with SessionLocal() as s:
        ...
        if not rows:
            return html.Div(className="empty-state", children=[
                html.H3("No Offices Yet"),
                html.Div("Create your first office in the Admin panel.", className="muted")
            ])
...
def add_office(n, office_name, location):
    ...
    with SessionLocal() as s:
        if not name:
            return html.Div("❌ Office name is required.", className="message message-error")
        if s.query(Office).filter(Office.name.ilike(name)).first():
            return html.Div("❌ Office already exists.", className="message message-error")
        s.add(Office(name=name, location=loc))
        s.commit()
    return html.Div("✅ Office created successfully!", className="message message-success")

@app.callback(
    Output("msg-create-emp","children"),
    Output("admin-employees-table","children", allow_duplicate=True),
    Input("btn-create-emp","n_clicks"),
    State("emp-name","value"),
    State("emp-username","value"),
    State("emp-password","value"),
    State("emp-phone","value"),
    State("emp-role","value"),
    State("emp-office","value"),
    prevent_initial_call=True
)
@login_required(Role.GM)
def create_employee(n, name, username, password, phone, role, office_id):
    name = (name or "").strip()
    username = (username or "").strip()
    password = (password or "").strip()
    phone = (phone or "").strip() if phone else None
    
    if not name or not username or not password or not role or not office_id:
        return html.Div("❌ All required fields must be filled.", className="message message-error"), render_admin_employees_table()
    
    with SessionLocal() as s:
        # Check if username already exists
        if s.query(User).filter(User.username == username).first():
            return html.Div("❌ Username already exists.", className="message message-error"), render_admin_employees_table()
        
        # Check if office exists
        office = s.get(Office, office_id)
        if not office:
            return html.Div("❌ Invalid office selected.", className="message message-error"), render_admin_employees_table()
    ...
    return html.Div("✅ Employee created successfully!", className="message message-success"), render_admin_employees_table()

@app.callback(Output("admin-employees-table","children"), Input("url","pathname"))
@login_required(Role.GM)
def render_admin_employees_table(_=None):
    ...
    with SessionLocal() as s:
        ...
        for emp in employees:
            office = s.get(Office, emp.office_id) if emp.office_id else None
            rows.append({
                "employee_id": emp.id,
                "employee_name": emp.name,
                "username": emp.username or "—",
                "role": emp.role.value if emp.role else "—",
                "office_id": emp.office_id or "—",
        ...
        if not rows:
            return html.Div(className="empty-state", children=[
                html.H3("No Employees Yet"),
                html.Div("Create your first employee using the form above.", className="muted")
            ])
    ...
    with SessionLocal() as s:
        if user.role == Role.GM:
            ...
            return html.Div([
                ...
                html.Div(className="hr"),
                html.H4("🏢 Per-Office Analytics", style={"marginBottom": "12px"}),
                ...
                html.Div(className="hr"),
                html.H4("👤 Per-Employee Analytics", style={"marginBottom": "12px"}),
                ...
                html.Div(className="hr"),
                html.H4("💬 Add Remark for Employee", style={"marginBottom": "12px"}),
...
def per_office_kpis(office_id):
    ...
    return html.Div(className="card", style={"padding": "20px", "marginTop": "12px"}, children=[
        html.Div(className="kpi", style={"minWidth": "auto", "margin": "8px 0"}, children=[
            html.Div("Assets in Office", className="label"),
            html.Div(count, className="value")
        ]),
        html.Div(className="kpi", style={"minWidth": "auto", "margin": "8px 0"}, children=[
            html.Div("Pending Returns", className="label"),
            html.Div(pending, className="value")
        ]),
        html.Div(className="kpi", style={"minWidth": "auto", "margin": "8px 0"}, children=[
            html.Div("Total Cost", className="label"),
            html.Div(f"${cost:,.2f}", className="value")
        ])
    ])
...
def per_employee_kpis(emp_id):
    ...
    return html.Div(className="card", style={"padding": "20px", "marginTop": "12px"}, children=[
        html.Div(className="kpi", style={"minWidth": "auto", "margin": "8px 0"}, children=[
            html.Div("Resources Assigned", className="label"),
            html.Div(count, className="value")
        ]),
        html.Div(className="kpi", style={"minWidth": "auto", "margin": "8px 0"}, children=[
            html.Div("Pending Returns", className="label"),
        ...
        html.Div(className="kpi", style={"minWidth": "auto", "margin": "8px 0"}, children=[
            html.Div("Total Asset Cost", className="label"),
            html.Div(f"${cost:,.2f}", className="value")
        ])
    ])
