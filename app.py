...
def _safe_add_column(table, coldef):
    try:
        with engine.begin() as conn:
            ...
            """), {"table_name": table, "col_name": cname}).fetchone()
            if not result:
                conn.execute(text(f"ALTER TABLE {table} ADD COLUMN {coldef}"))
    except Exception:
        pass
...
# Initialize DB
# Check if database is already seeded by trying to query offices
try:
    with SessionLocal() as s:
        if s.query(Office).first():
            init_db(seed=False)  # Database exists, just ensure schema is up to date
        else:
            init_db(seed=True)   # Empty database, seed it
except Exception as e:
    # If connection fails, we'll get an error on first use anyway
    # This allows the app to start even if DB isn't ready yet
    print(f"⚠️ Database initialization check failed: {e}")
    init_db(seed=False)  # Try to create schema anyway
